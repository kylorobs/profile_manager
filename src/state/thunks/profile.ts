...
export const uploadData: any = createAsyncThunk(
  'profiles/uploadData',
  async (data: any) => {

    try {
      const url = store.getState().data.dataUrl;
      await makeRequest('PUT', createUrl({ url }), data);
      store.dispatch(fetchData(url));
    }
    catch (e) {
      store.dispatch(setError('Error uploading the new data to the database.' + e));
      store.dispatch(notLoading());
      return;
    }
  }
